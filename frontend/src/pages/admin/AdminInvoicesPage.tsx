...
const STATUS_COLORS: Record<string, string> = {
  ...
  partially_paid: 'bg-amber-900/40 text-amber-400',
  cancelled: 'bg-gray-700 text-gray-400',
  returned: 'bg-purple-900/40 text-purple-400',
};
...
export default function AdminInvoicesPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      const res = await adminApi.get(`/api/admin/invoices?${params}`);
      setInvoices(res.data.invoices);
      setTotal(res.data.pagination.total);
      setTotalPages(res.data.pagination.totalPages);
    } catch (err) {
      setError(getAdminApiError(err, 'Failed to load invoices'));
    } finally {
      setLoading(false);
    }
  }, [page, statusFilter]);
    ...
    <div className="p-6 space-y-5">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 text-sm">{total} invoices across all merchants</p>
        </div>
        <select
          value={statusFilter}
      ...
      <div className="bg-gray-900 rounded-xl border border-gray-800 overflow-hidden">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-800 bg-gray-800/50">
                ...
                <th className="text-right px-4 py-3 text-gray-400 font-medium">Paid</th>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={8} className="text-center py-12 text-gray-500">No invoices found</td></tr>
              ) : (
                invoices.map((inv) => (
                  <tr key={inv._id} className="border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors">
                    <td className="px-4 py-3 font-mono text-xs text-white">{inv.invoiceNumber}</td>
                    <td className="px-4 py-3">
                      <div className="text-gray-300 text-xs">{inv.merchantId?.businessName || '—'}</div>
                    </td>
                    <td className="px-4 py-3">
                      <div className="text-gray-300">{inv.customerId?.name || '—'}</div>
                    </td>
                    <td className="px-4 py-3">
                      <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${STATUS_COLORS[inv.status] ?? 'bg-gray-700 text-gray-400'}`}>
                        {inv.status.replace('_', ' ')}
                      </span>
                    </td>
                    <td className="px-4 py-3 text-gray-400 text-xs capitalize">{inv.paymentMode}</td>
                    <td className="px-4 py-3 text-right text-white font-medium">{fmt(inv.totalAmount)}</td>
                    <td className="px-4 py-3 text-right text-green-400">{fmt(inv.paidAmount)}</td>
                    <td className="px-4 py-3 text-gray-400 text-xs">{new Date(inv.createdAt).toLocaleDateString('en-IN')}</td>
                  </tr>
                ))
              )}
            </tbody>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between px-4 py-3 border-t border-gray-800">
            <p className="text-sm text-gray-400">Page {page} of {totalPages} · {total} invoices</p>
